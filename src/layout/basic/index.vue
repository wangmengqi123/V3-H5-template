<template>
  ...
  <!-- <nut-tabbar unactive-color="#364636" active-color="#1989fa" bottom v-model="activeTab" v-show="tabbarVisible" @tab-switch="tabSwitch">
    <nut-tabbar-item v-for="item in tabItem" :key="item.key" :tab-title="item.key" :icon="item.icon" />
  </nut-tabbar> -->
  <nut-tabbar unactive-color="#364636" active-color="#1989fa" bottom @tab-switch="tabSwitch" v-model="activeTab"> 
    <nut-tabbar-item v-for="(item, index) in tabBarList" :tab-title="item.title" :icon="item.icon" :key="index">
    </nut-tabbar-item>
  </nut-tabbar>
</template>
...
import { Home, Horizontal, My, Location } from '@nutui/icons-vue'; //tabbar图标
